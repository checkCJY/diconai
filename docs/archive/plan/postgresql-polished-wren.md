...
=======
-- 1.2 gas_data 9개 가스 컬럼 NaN/Inf (co, h2s, co2, o2, no2, so2, o3, nh3, voc)
SELECT COUNT(*) FROM gas_data
...
=======
1. **이관 비용 vs 가치 불균형**: power_data 현재 9.2M행 (보존정책 미적용 누적, 2026-05-22 측정). W2 진입 시 **선택지 2가지** — (a) `data_retention` 1회 강제 실행 후 이관(권장, 14일 윈도우로 축소되어 수십만~수 M 수준), (b) 현재 SQLite를 `pg_dump`/파일 백업해두고 Raw 전체 이관 제외 → PG 신규 DB에 새로 쌓기. 어느 쪽이든 14일 보존정책상 시연일까지 데이터가 자연 회전하므로 Raw 이관 가치 낮음.
>>>>>>> Stashed changes

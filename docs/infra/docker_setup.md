...
> `INTERNAL_SERVICE_TOKEN`과 `DRF_SERVICE_TOKEN`은 같은 값. fastapi→drf 헤더용 변수가 분리되어 있을 뿐.
...
python3 -c "import secrets; print(secrets.token_urlsafe(32))"   # INTERNAL_SERVICE_TOKEN
python3 -c "import secrets; print(secrets.token_urlsafe(32))"   # JWT_SIGNING_KEY

# .env.docker 편집 → 위 3개 값 + DRF_SERVICE_TOKEN(=INTERNAL_SERVICE_TOKEN 값)
nano .env.docker
```

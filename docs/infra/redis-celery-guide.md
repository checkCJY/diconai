...
| Pub/Sub 쓰나? | **안 씀.** WS 알람 전달은 Redis **List 큐** (LPUSH/BRPOP) |
...
### 2.3. WS 알람 큐 (Raw Redis List, Django cache 우회)
...
MAX_QUEUE_LEN = 10_000                  # LTRIM으로 폭주 시 가장 오래된 알람 drop
```

| 연산 | 위치 | 의미 |
|---|---|---|
| `LPUSH diconai:ws:alarms` | [push_alarm()](../../fastapi-server/websocket/services/alarm_queue.py) | 알람 1건을 큐 좌측에 추가 |
| `BRPOP diconai:ws:alarms 0` | [alarm_flush_loop()](../../fastapi-server/websocket/routers/ws_router.py) | 큐 우측에서 즉시 pop (timeout=0 = 무한 대기) |
| `LTRIM ... 0 9999` | push_alarm 직후 | 폭주 보호 |
| `LLEN` | metrics export | `ALARM_QUEUE_LENGTH` Gauge |

**왜 Pub/Sub가 아니라 List 큐인가**: 알람은 손실 금지 — Pub/Sub의 "구독자 없으면 즉시 손실" 특성과 충돌. List는 fastapi 일시 다운 시에도 큐에 적체 → 복귀 후 처리.
...
| 키 | 연산 | TTL | 용도 |
|---|---|---|---|
| `alarm:push:dedup:{fingerprint}` | `SET NX EX` | 60s | 첫 도착자만 LPUSH 통과 |
...
|---|---|---|---|---|
| `diconai:ws:alarms` | List | — | WS 알람 큐 | fastapi alarm_queue.py |
...
[7] fastapi push_alarm_handler()
    └─ alarm_queue.push_alarm()
        ...
        ├─ LPUSH "diconai:ws:alarms"
        └─ LTRIM 0 9999 (폭주 보호)
                                  ↓
[8] alarm_flush_loop (백그라운드)
    └─ BRPOP "diconai:ws:alarms" 0 (무한 대기)
        └─ sensor_clients 전체에 WebSocket broadcast
                                  ↓
[9] 브라우저 토스트 표시
```
...
process_anomaly_inference()
   ...
   │    └─ anomaly_alarm.push_anomaly_alarm() → LPUSH "diconai:ws:alarms"
   ↓ (별도 흐름)
...
| ③ Push fingerprint | fastapi alarm_queue.push_alarm | `alarm:push:dedup:*` | Celery retry로 인한 중복 LPUSH |
...
| 메트릭 | 의미 | 정상 범위 |
|---|---|---|
| `ALARM_QUEUE_LENGTH` | `LLEN diconai:ws:alarms` | < 100 |
| `ALARM_WS_PUSH_FAILED_TOTAL` | DRF→fastapi push 실패 | 0 증가 |
| `REDIS_COMMAND_DURATION{cmd}` | LPUSH/BRPOP 등 명령 지연 | p95 < 10ms |
...
**증상: 알람이 브라우저에 안 뜬다**
1. `redis-cli LLEN diconai:ws:alarms` — 큐에 쌓여 있는지?
...
| WS 알람 큐 | LPUSH 실패 → fastapi `/internal/alarms/push/` 503 | DRF Celery가 max_retries=3로 재시도 |
...
| `ALARM_QUEUE_LENGTH` | > 100 지속 | BRPOP consumer 추가 필요 |
| broadcast latency p95 | > 500ms | 다중 fastapi + Redis Streams (Consumer Group) 검토 |
| `CELERY_TASK_QUEUED` p95 | > 1s | worker concurrency ↑ 또는 큐 분리 |

**Phase 2 마이그레이션 시 변경 예상 지점**:
- WS 알람 큐 `List` → `Streams + Consumer Group` (다중 fastapi가 모두 broadcast)

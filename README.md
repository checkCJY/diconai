...
| **정휘훈** | 백오피스 · 모니터링 | 정휘훈 | 백오피스 · 모니터링 · 성능개선 | 어드민 기준정보 CRUD 백엔드(공지/코드/위험기준/임계치/로그), 대시보드 가스 패널, Grafana(전력/가스 AI), Prometheus 메트릭, Dev/Prod 설정 분리, 부하 테스트 스크립트 작성 및 병목 진단, Celery 성능 개선(p95 460ms→49ms)
 |

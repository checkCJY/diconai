...
def test_w_warning_then_a_danger_fires_once_each(device_with_meta):
    """W WARNING(82%) → A DANGER(106%) — 각 transition 마다 fire 1회씩."""
    with (
        patch(
        ...
        # 1) W 820W (82% of 1000) → aggregate WARNING
        trigger_power_alarms(
        ...
        # 2) A 10.6A (106% of 10) → aggregate DANGER (W still WARNING cached)
        trigger_power_alarms(
            _make_objs(device_with_meta, "current", 1, 10.6), device_with_meta
        )
        assert fire_danger.delay.call_count == 1

        # 3) V 350V (92% of 380, 저전압 WARNING) → aggregate 여전히 DANGER, 추가 fire 없음
        trigger_power_alarms(
...
@pytest.mark.django_db
def test_voltage_low_alone_fires_danger(device_with_meta):
    """저전압 단독(342V = 90% of 380) → DANGER 발화. 이전엔 못 잡던 위험."""
    with patch(
        "apps.monitoring.services.power_alarm.fire_power_danger_task"
    ) as fire_danger:
        trigger_power_alarms(

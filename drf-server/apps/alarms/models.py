...
class AlarmRecord(models.Model):
    ...
    facility = models.ForeignKey(
        "sensors.Facility", on_delete=models.CASCADE, related_name="alarm_records"
    )
    ...
    alarm_type = models.CharField(max_length=100, choices=AlarmType.choices)
    ...
    alarm_level = models.CharField(max_length=10, choices=AlarmLevel.choices)
...
class Notification(models.Model):
    ...
    alarm = models.ForeignKey(
        AlarmRecord, on_delete=models.CASCADE, related_name="notifications"
    )
...
class SafetyCheckItem(models.Model):
    ...
    facility = models.ForeignKey(
        "sensors.Facility", on_delete=models.CASCADE, related_name="safety_check_items"
    )
    title = models.CharField(max_length=100)
    ...
    class Meta:
        ...
        constraints = [
            models.UniqueConstraint(
                fields=["facility", "order"], name="uq_safety_check_item_facility_order"
            )
        ]
...
class SafetyStatus(models.Model):
    ...
    worker = models.ForeignKey(
        "accounts.CustomUser", on_delete=models.CASCADE, related_name="safety_statuses"
    )
    check_item = models.ForeignKey(
        SafetyCheckItem, on_delete=models.CASCADE, related_name="safety_statuses"
    )
    ...
    class Meta:
        ...
        constraints = [
            models.UniqueConstraint(
                fields=["worker", "check_item"], name="uq_safety_status_worker_item"
            )
        ]

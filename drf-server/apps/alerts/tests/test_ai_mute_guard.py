...
def clear_cache(settings):
    ...
    DANGER_CONFIRM_TICKS=1 로 고정 — 본 파일은 AI mute 가드(룰 fire suppress)를
    단일 틱으로 검증한다. 2틱 confirm(#110)이 걸리면 mute 가드 이전에 held 되어
    suppress 카운터에 도달하지 못한다. confirm 게이트는 test_danger_confirm.py 가 별도 커버.
    """

...
class AlarmRecord(BaseModel):
    ...
    [역할 축소]
    is_active, status, resolved_by, resolved_at 같은 업무 상태는 Event 가 관리하고
    AlarmRecord 는 판정의 순간적 사실만 기록한다 (불변).
    ...
    # AI 알람 한정 algorithm 출처 라벨. power/gas anomaly_ai 알람만 채워지고
    # 룰 기반은 빈 문자열 또는 NULL.
    # 값: "isolation_forest" | "arima" | "combined" | "night_abnormal" | "" | NULL
    # null=True 인 이유: Django SQLite ALTER TABLE ADD COLUMN 이 column DEFAULT 를
    # 미적용하는 이슈 대비. ORM 흐름은 default="" 그대로, raw SQL / 옛 ORM 캐시
    # INSERT 시 NULL 도 허용해 IntegrityError 방지. NULL/'' 둘 다 "AI 알람 아님"
    # 의미로 동일 취급 (filter 시 isnull=True or exact="" 양쪽 고려).
    ...
    # 검출 주체 (AI vs 정적룰). algorithm_source 와 직교 차원 — 같은 행에 동시
    # 존재 가능. decide_alarm 매트릭스가 결정. static_legacy 는 옛 데이터 backfill
    # 전용 — 신규 알람에 사용 금지. null=True 는 SQLite ALTER TABLE 안전 목적.
    ...
    def get_short_message(self) -> str:
        ...
        if self.power_device_id and self.measured_value is not None:
            ...
            if self.alarm_type == "power_anomaly_ai":
                # algorithm_source 별 운영자 친화 워딩. 칩 없이 텍스트로 알고리즘
                # 본질 구분 (IF=수치, ARIMA=패턴, combined=동시 등).
            ...
            # "정적" 기술 용어 제거 — 가스("CO 임계치 초과")와 톤 통일. "정적룰 vs
            # AI" 분류는 source 컬럼 + cover-badge 로 표현되니 본문은 도메인 사실만.
            return f"{prefix}임계치 초과 ({self.measured_value:,.1f} W)"
        if self.geofence_id:
            # 작업자 이름 컨텍스트 추가. 모달 sensor 영역에 이미 geofence 이름
            # (source_label) 있음 — 본문은 "누가" 들어갔는지가 운영자가 가장 알고
            # 싶은 정보. worker FK 미존재 시 기존 단순 텍스트.
            if self.worker_id and self.worker and self.worker.name:
                return f"{self.worker.name} 작업자 진입"
            return "위험구역 진입"
        if self.alarm_type == "sensor_fault":
            return "센서 통신 이상"
        # 정상화 메시지에도 device/gas prefix 포함 → 운영자가 어느 센서가 복귀했는지
        # 즉시 인지. push payload 의 message 필드 단일 진실 공급원.

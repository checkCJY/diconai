...
class DataRetentionPolicyListView(APIView):
    ...
    def get(self, request):
        policies = DataRetentionPolicy.objects.all().order_by("device_type", "data_category")
...
class DataRetentionPolicyPreviewView(APIView):
    ...
    def get(self, request, pk):
        ...
        try:
            history_days = int(history_days_str) if history_days_str else policy.history_retention_days
            if history_days < 1:
                raise ValueError
        except (TypeError, ValueError):
            return Response(
        ...
        return Response(
            {
                ...
                "affected_rows": count,
                "days_reduced": (
                    raw_days < policy.raw_retention_days or
                    history_days < policy.history_retention_days
                ),
            }
        )

...
> **후속 변경 (2026-05-18 W4, commit `2df4fe4`)** — `AI 이상 패턴 감지` 문구가 algorithm 출처 라벨 (`IF` / `ARIMA` / `IF+ARIMA` / `야간 가동`) prefix 로 대체됨. 본 commit 8976109 의 채널 라벨 (`prefix`) 흐름은 그대로 유지되고, AI 알람 분기 안에서 `ALGORITHM_SOURCE_LABEL` 매핑 한 단계 추가:
> ```python
> if self.alarm_type == "power_anomaly_ai":
...
> ```
> 즉 본 commit 결과 `"송풍기A AI 이상 패턴 감지 (15.58 W)"` 는 W4 후속 작업 후 `"송풍기A IF+ARIMA 이상 감지 (15.58 W)"` 같은 더 구체적 라벨로 진화. 채널·channel_meta 흐름은 변경 없음.
...
- (W4 후속) AI 알람 + channel_meta 등록 시: `"송풍기A IF+ARIMA 이상 감지 (7925.8 W)"`
...
본 세션 작업 후 같은 영역에 영향을 준 후속 commit. 시간순으로 누가 무엇을 바꿨는지 추적.

### 2026-05-18 W4 — `AlarmRecord.algorithm_source` 도입 (ARIMA Un-격하 plan §8)

본 세션의 `AlarmRecord.channel` 필드 옆에 W4 작업자가 `algorithm_source` 필드를 추가. AI 알람 (`power_anomaly_ai` / `gas_anomaly_ai`) 의 출처 (`isolation_forest` / `arima` / `combined` / `night_abnormal` / `""`) 를 저장.

**B.3 와의 연관**:
- `get_short_message` 의 `power_anomaly_ai` 분기에 `algorithm_source` 라벨 prefix 가 추가됨 — 채널 prefix(`송풍기A`) 다음·"이상 감지" 앞에 algorithm 라벨(`IF+ARIMA`) 삽입
- 본 세션의 채널 prefix 흐름은 그대로 — W4 가 "AI 이상 패턴 감지" 단일 문자열만 algorithm 매핑으로 분기
- 가스 알람(`gas_anomaly_ai`) 도 같은 패턴 — `"CO IF 이상 감지 (290.0 ppm)"`

**관련 commit**:
- `2205a13` feat(alerts): W4 — `AlarmRecord.algorithm_source` + migration 0017 + AI_TO_RULE_LEVEL 정리
- `2df4fe4` feat(alerts): AI 추론 가시성 — `get_short_message` algorithm 라벨 + serializer 노출 + WS push payload 보강
- `ccd15aa` fix(ai): ARIMA 실동작 보강 + 토스트 algorithm 라벨

**관련 파일 (W4 추가분)**:
- `drf-server/apps/alerts/migrations/0017_alarmrecord_algorithm_source.py` (신규)
- `drf-server/apps/alerts/models/alarm_record.py` — `algorithm_source` 필드 + `get_short_message` 분기 보강
- `drf-server/apps/alerts/serializers/alarm_record.py` — `algorithm_source` 노출 (UI 칩 표시용)
- `drf-server/apps/alerts/serializers/anomaly_alarm_record.py` — `algorithm_source` 입력 필드
- `drf-server/apps/alerts/views/anomaly_alarm_record.py` — `create_alarm_and_event` 호출에 algorithm_source 전달
- `drf-server/apps/alerts/services/event_service.py` — `create_alarm_and_event(..., algorithm_source=)` 인자
- `drf-server/apps/core/constants.py` — `ALGORITHM_SOURCE_LABEL` 매핑 신설
- `fastapi-server/power/services/power_service.py` — algorithm_source 결정 (priority: night_abnormal > combined > arima > isolation_forest) + alarm_payload / push_payload 전달

**본 세션 인용 코드 갱신 — B.3 안 inline 보강분 참조** (메시지 섹션의 후속 변경 박스).

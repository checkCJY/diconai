...
logs-celery:  ## celery 워커 2종 (alarm: 알람·ML forward / metric: 주기 메트릭)
	docker compose logs -f --tail=100 celery-worker-alarm celery-worker-metric
...
logs-all:     ## drf + fastapi + celery-worker-alarm + beat (요청→저장→알람 흐름 한 화면)
	docker compose logs -f --tail=50 drf fastapi celery-worker-alarm celery-beat
...
logs-locks:   ## DB 락/데드락 감시 (PG: deadlock·lock timeout — 락 경합 회귀 확인)
	docker compose logs -f --tail=100 celery-worker-alarm celery-worker-metric 2>&1 | grep -E --line-buffered "deadlock|lock timeout|could not obtain lock|LockNotAvailable"
...
demo-prep:       ## 시연 직전 일괄 셋업 (mode=normal + Redis 키 정리 + window=30 안정화 대기)
	...
	@t=0; while [ $$t -lt 35 ] && [ "$$(docker compose exec redis redis-cli --scan --pattern 'ai_fired_gas:*' 2>/dev/null | wc -l)" -gt 0 ]; do printf "."; sleep 5; t=$$((t+5)); done; echo " ✓ ($${t}s)"

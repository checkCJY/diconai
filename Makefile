...
# compose 호출 — .env.docker 를 변수 보간 소스로 명시 (시크릿을 yaml 밖에 둠)
...
.PHONY: help up down start stop restart ps build rebuild logs sh exec \
        ...
        demo-gas demo-power demo-cycle
...
help:  ## 전체 명령어 + 자주 쓰는 시나리오 보기
	...
	@printf "      make logs                            $(DIM)# 12개 서비스 전체 (stdout, 휘발성)$(RESET)\n"
	...
	@printf "  $(DIM)인자: s=서비스명(drf|fastapi|postgres|redis|celery-worker-alarm|celery-worker-metric|celery-beat|prometheus|grafana)$(RESET)\n\n"


##@ 🐳 빌드 / 기동

up:           ## 12개 서비스 전체 기동 (백그라운드). 이미지 없으면 자동 빌드
	$(DC) up -d
...
ps:           ## 12개 서비스 상태 표 (STATUS가 healthy/Up인지 확인)
	$(DC) ps
...
logs:         ## 로그 실시간 (Ctrl+C). s=서비스명 미지정이면 전체 12개 서비스
	$(DC) logs -f --tail=100 $(s)

logs-drf:     ## drf 단독 (gunicorn access + Django request/ERROR)
	$(DC) logs -f --tail=100 drf

logs-fastapi: ## fastapi 단독 (uvicorn access + IoT 인입 + WS)
	$(DC) logs -f --tail=100 fastapi

logs-celery:  ## celery 워커 2종 (alarm: 알람·ML forward / metric: 주기 메트릭)
	$(DC) logs -f --tail=100 celery-worker-alarm celery-worker-metric

logs-beat:    ## celery-beat 단독 (retention 등 주기 태스크 스케줄링)
	$(DC) logs -f --tail=100 celery-beat

logs-all:     ## drf + fastapi + celery-worker-alarm + beat (요청→저장→알람 흐름 한 화면)
	$(DC) logs -f --tail=50 drf fastapi celery-worker-alarm celery-beat
...
logs-locks:   ## DB 락/데드락 감시 (PG: deadlock·lock timeout — 락 경합 회귀 확인)
	$(DC) logs -f --tail=100 celery-worker-alarm celery-worker-metric 2>&1 | grep -E --line-buffered "deadlock|lock timeout|could not obtain lock|LockNotAvailable"
...
logs-retention: ## retention task 발사·실행 로그 (매일 09:30 KST, metric 워커)
	docker logs -f diconai-celery-worker-metric-1 2>&1 | grep -E --line-buffered "retention|run_data_retention"
...
shell-drf:    ## Django shell 진입 (ORM 조작, 디버깅용)
	$(DC) exec drf python manage.py shell


##@ 🗄 Django DB 작업

migrate:      ## 미적용 마이그레이션 수동 실행 (entrypoint가 자동 처리하지만 명시적으로)
	$(DC) exec drf python manage.py migrate

showmigrations: ## 마이그레이션 적용 상태 ([X] 적용 / [ ] 미적용)
	$(DC) exec drf python manage.py showmigrations

super:        ## 슈퍼유저 생성 (대화형: id/이메일/비밀번호 입력)
	$(DC) exec drf python manage.py createsuperuser

seed:         ## 더미 데이터 시드 (Worker × 4, GasSensor, PowerDevice 등)
	$(DC) exec drf python manage.py seed_dummy_data
...
targets:      ## Prometheus가 scrape하는 6개 target 상태 (drf·fastapi·node·postgres·redis·prometheus, 모두 health=up)
...
##@ 💾 DB 상태 (PostgreSQL — 운영 점검·디버깅)

define _DB_SETTINGS_PY
import django, os
os.environ.setdefault('DJANGO_SETTINGS_MODULE', 'config.settings')
django.setup()
from django.db import connection
with connection.cursor() as c:
    for s in ('server_version', 'max_connections', 'statement_timeout',
              'idle_in_transaction_session_timeout', 'default_transaction_isolation'):
        c.execute('SHOW ' + s)
        print(f'  {s:<36}: {c.fetchone()[0]}')
endef
export _DB_SETTINGS_PY
...
with connection.cursor() as c:
    ...
    print(f'  database total           : {c.fetchone()[0]}')
    c.execute('SELECT relname, pg_size_pretty(pg_total_relation_size(relid)) '
              'FROM pg_catalog.pg_statio_user_tables '
              'ORDER BY pg_total_relation_size(relid) DESC LIMIT 8')
    for n, s in c.fetchall():
        print(f'  {n:<24} : {s}')
endef
export _DB_SIZE_PY
...
db-size:          ## DB 총 크기 + 상위 테이블 크기 (비대화 폭증 감지)
	@docker exec diconai-drf-1 python -c "$$_DB_SIZE_PY"

db-pragma:        ## PG 주요 설정 (statement_timeout·isolation 등 검증)
	@docker exec diconai-drf-1 python -c "$$_DB_SETTINGS_PY"
...
scenario-clean:  ## 알람 큐 + dedup/상태 키 일괄 정리 (리허설 사이 초기화)
	@$(DC) exec redis redis-cli DEL diconai:ws:alarms > /dev/null
	@for pat in 'alarm:state:*' 'alarm:power:*' 'ai_fired:*' 'ai_fired_gas:*' 'ai_state_gas:*' 'alarm:push:dedup:*'; do \
	  $(DC) exec redis redis-cli --scan --pattern "$$pat" | \
	    xargs -r -I {} $(DC) exec redis redis-cli DEL {} > /dev/null; \
	done
	@echo "✅ Redis 알람 관련 키 모두 정리됨"

demo-prep:       ## 시연 직전 일괄 셋업 (mode=normal + Redis 키 정리 + window=30 안정화 대기)
	@echo "🎬 시연 준비..."
	@$(MAKE) -s scenario-set mode=normal
	@$(MAKE) -s scenario-clean
	@printf "⏳ window=30 RAMP_DOWN transient 안정화 대기 (ai_fired_gas:* 잔존 확인)..."
	@t=0; while [ $$t -lt 35 ] && [ "$$($(DC) exec redis redis-cli --scan --pattern 'ai_fired_gas:*' 2>/dev/null | wc -l)" -gt 0 ]; do printf "."; sleep 5; t=$$((t+5)); done; echo " ✓ ($${t}s)"
...
demo-check:      ## 시연 환경 한방 점검 (현재 모드 + env + 큐 길이)
	...
	@$(MAKE) -s scenario
	@printf "  • env: "
	@$(DC) exec fastapi printenv DUMMY_SEND_INTERVAL_SEC DUMMY_SCENARIO_MODE 2>&1 | tr '\n' ' '; echo
	@printf "  • 알람 큐 길이: "
	@$(DC) exec redis redis-cli LLEN diconai:ws:alarms

demo-gas:        ## 가스 시연 1 사이클 (co_leak 60s + normal 복귀). 추가 사이클 차단.
	@echo "🎬 가스 시연 시작 (co_leak)"
	@$(MAKE) -s scenario-set mode=co_leak > /dev/null
	@echo "⏳ 60초 진행 — RAMP_UP → DANGER 격상 → HOLD"
	@sleep 60
	@$(MAKE) -s scenario-reset > /dev/null
	@echo "✅ 가스 시연 1 사이클 완료 (normal 복귀)"

demo-power:      ## 전력 시연 1 사이클 (overload 60s + normal 복귀).
	@echo "🎬 전력 시연 시작 (overload)"
	@$(MAKE) -s scenario-set mode=overload > /dev/null
	@echo "⏳ 60초 진행 — AI 채널 1개 격상 + mute 가드"
	@sleep 60
	@$(MAKE) -s scenario-reset > /dev/null
	@echo "✅ 전력 시연 1 사이클 완료"

demo-cycle:      ## A(가스) + B(전력) 통합 시연 (~2분 30초)
	@$(MAKE) -s demo-prep > /dev/null
	@$(MAKE) -s demo-gas
	@echo "--- 전환 30초 ---"
	@sleep 30
	@$(MAKE) -s demo-power
